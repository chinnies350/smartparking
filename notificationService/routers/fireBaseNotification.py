...
async def postFireBaseNotification(request:schemas.FireBaseNotification, db: Cursor = Depends(get_cursor)):
    try:
        url=f"{os.getenv('USER_SERVICE_URL')}/userMaster?userId={request.userId}"
        response = requests.get(url)
        response = json.loads(response.text)
        if response['statusCode']==1:
            res=send_topic_push(response['response'][0]['registrationToken'],request.title, request.body, request.userId)
            return res
        else:
            return {
